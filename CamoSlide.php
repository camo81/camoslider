...
class CamoSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'camoslider` (`id_shop`, `id_homeslider_slides`)
	...
	public function delete()
	{
		...
		foreach ($images_mobile as $image_mobile)
		{
			if (preg_match('/sample/', $image_mobile) === 0)
				if ($image_mobile && file_exists(dirname(__FILE__).'/images/'.$image_mobile))
					$res &= @unlink(dirname(__FILE__).'/images/'.$image_mobile);
		}

		$res &= $this->reOrderPositions();

		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'camoslider`
	...
	public function reOrderPositions()
	{
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM `'._DB_PREFIX_.'camoslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'camoslider` hs ON (hss.`id_homeslider_slides` = hs.`id_homeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new CamoSlide($row['id_slide']);
