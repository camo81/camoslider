...
include_once(_PS_MODULE_DIR_.'camoslider/CamoSlide.php');

class CamoSlider extends Module
{
	protected $_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Camo Image slider for your homepage');
	...
	public function install()
	{
		...
		)
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res = Configuration::updateValue('CAMOSLIDER_WIDTH', $this->default_width, false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('CAMOSLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('CAMOSLIDER_PAUSE', $this->default_pause, false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('CAMOSLIDER_LOOP', $this->default_loop, false, $shop_group_id, $shop_id);
			}
			...
			if (count($shop_groups_list))
			{
				...
				{
					$res = Configuration::updateValue('CAMOSLIDER_WIDTH', $this->default_width, false, $shop_group_id);
					$res &= Configuration::updateValue('CAMOSLIDER_SPEED', $this->default_speed, false, $shop_group_id);
					$res &= Configuration::updateValue('CAMOSLIDER_PAUSE', $this->default_pause, false, $shop_group_id);
					$res &= Configuration::updateValue('CAMOSLIDER_LOOP', $this->default_loop, false, $shop_group_id);
				}
			}

			/* Sets up Global configuration */
			$res = Configuration::updateValue('CAMOSLIDER_WIDTH', $this->default_width);
			$res &= Configuration::updateValue('CAMOSLIDER_SPEED', $this->default_speed);
			$res &= Configuration::updateValue('CAMOSLIDER_PAUSE', $this->default_pause);
			$res &= Configuration::updateValue('CAMOSLIDER_LOOP', $this->default_loop);
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName('CAMOSLIDER_WIDTH');
			$res &= Configuration::deleteByName('CAMOSLIDER_SPEED');
			$res &= Configuration::deleteByName('CAMOSLIDER_PAUSE');
			$res &= Configuration::deleteByName('CAMOSLIDER_LOOP');
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'camoslider` (
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'camoslider_slides` (
			  `id_homeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'camoslider_slides_lang` (
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new CamoSlide($slide['id_slide']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'camoslider`, `'._DB_PREFIX_.'camoslider_slides`, `'._DB_PREFIX_.'camoslider_slides_lang`;
		');
	}
	...
	public function getContent()
	{
		...
		elseif (Tools::isSubmit('addSlide') || (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide'))))
		{
			...
			{
				$associated_shop_ids = CamoSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
	...
	protected function _postValidation()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			...
			if (!Validate::isInt(Tools::getValue('CAMOSLIDER_SPEED')) || !Validate::isInt(Tools::getValue('CAMOSLIDER_PAUSE')) ||
				!Validate::isInt(Tools::getValue('CAMOSLIDER_WIDTH'))
			)
		...
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_'.$id_lang_default]) || empty($_FILES['image_'.$id_lang_default]['tmp_name'])))
				...
				if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_mobile_'.$id_lang_default]) || empty($_FILES['image_mobile_'.$id_lang_default]['tmp_name'])))
					$errors[] = $this->l('The image is not set.');
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res = Configuration::updateValue('CAMOSLIDER_WIDTH', (int)Tools::getValue('CAMOSLIDER_WIDTH'), false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('CAMOSLIDER_SPEED', (int)Tools::getValue('CAMOSLIDER_SPEED'), false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('CAMOSLIDER_PAUSE', (int)Tools::getValue('CAMOSLIDER_PAUSE'), false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('CAMOSLIDER_LOOP', (int)Tools::getValue('CAMOSLIDER_LOOP'), false, $shop_group_id, $shop_id);
			}
			...
			switch ($shop_context)
			{
				case Shop::CONTEXT_ALL:
					$res = Configuration::updateValue('CAMOSLIDER_WIDTH', (int)Tools::getValue('CAMOSLIDER_WIDTH'));
					$res &= Configuration::updateValue('CAMOSLIDER_SPEED', (int)Tools::getValue('CAMOSLIDER_SPEED'));
					$res &= Configuration::updateValue('CAMOSLIDER_PAUSE', (int)Tools::getValue('CAMOSLIDER_PAUSE'));
					$res &= Configuration::updateValue('CAMOSLIDER_LOOP', (int)Tools::getValue('CAMOSLIDER_LOOP'));
					if (count($shop_groups_list))
					{
						foreach ($shop_groups_list as $shop_group_id)
						{
							$res = Configuration::updateValue('CAMOSLIDER_WIDTH', (int)Tools::getValue('CAMOSLIDER_WIDTH'), false, $shop_group_id);
							$res &= Configuration::updateValue('CAMOSLIDER_SPEED', (int)Tools::getValue('CAMOSLIDER_SPEED'), false, $shop_group_id);
							$res &= Configuration::updateValue('CAMOSLIDER_PAUSE', (int)Tools::getValue('CAMOSLIDER_PAUSE'), false, $shop_group_id);
							$res &= Configuration::updateValue('CAMOSLIDER_LOOP', (int)Tools::getValue('CAMOSLIDER_LOOP'), false, $shop_group_id);
						}
					}
					break;
		...
		{
			$slide = new CamoSlide((int)Tools::getValue('id_slide'));
			if ($slide->active == 0)
				$slide->active = 1;
			else
				$slide->active = 0;
				...
				elseif (Tools::getValue('image_old_'.$language['id_lang']) != '')
					...
					)
					{
						...
						elseif (!$temp_name || !move_uploaded_file($_FILES['image_mobile_'.$language['id_lang']]['tmp_name'], $temp_name))
							return false;
						elseif (!ImageManager::resize($temp_name, dirname(__FILE__).'/images/'.$salt.'_'.$_FILES['image_mobile_'.$language['id_lang']]['name'], null, null, $type))
							$errors[] = $this->displayError($this->l('An error occurred during the image upload process.'));
						if (isset($temp_name))
							@unlink($temp_name);
		...
		{
			$slide = new CamoSlide((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	{
		if (!$this->isCached('camoslider.tpl', $this->getCacheId()))
		{
			...
			$this->smarty->assign(array('camoslider_slides' => $slides));
		}
	...
	public function hookdisplayHeader($params)
	{
		...
		$slider = array(
			'width' => $config['CAMOSLIDER_WIDTH'],
			'speed' => $config['CAMOSLIDER_SPEED'],
			'pause' => $config['CAMOSLIDER_PAUSE'],
			'loop' => (bool)$config['CAMOSLIDER_LOOP'],
		);

		$this->smarty->assign('camoslider', $slider);
	...
	public function hookdisplayTopColumn($params)
	{
		...
		return $this->display(__FILE__, 'camoslider.tpl', $this->getCacheId());
	}
	...
	{
		$this->_clearCache('camoslider.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'camoslider (id_homeslider_slides, id_shop)
			SELECT id_homeslider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'camoslider
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'camoslider_slides` hss, `'._DB_PREFIX_.'camoslider` hs
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM '._DB_PREFIX_.'camoslider hs
			LEFT JOIN '._DB_PREFIX_.'camoslider_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
			LEFT JOIN '._DB_PREFIX_.'camoslider_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)
	...
	public function getAllImagesMobileBySlidesId($id_slides, $active = null, $id_shop = null)
	{
		$this->context = Context::getContext();
		$images = array();
		...
		foreach ($results as $result)
			$images_mobile[$result['id_lang']] = $result['image_mobile'];

		return $images_mobile;
	}
	...
	public function renderList()
	{
		...
		foreach ($slides as $key => $slide)
		{
			...
			$associated_shop_ids = CamoSlide::getAssociatedIdsShop((int)$slide['id_slide']);
		...
		$fields_form = array(
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'file_lang',
						'label' => $this->l('Select a file for mobile'),
						'name' => 'image_mobile',
						'required' => true,
						'lang' => true,
						'desc' => sprintf($this->l('Maximum image size: %s.'), ini_get('upload_max_filesize'))
			...
			'form' => array(
				...
				'input' => array(
					array(
						...
						'name' => 'CAMOSLIDER_WIDTH',
						'suffix' => 'pixels'
					),
					array(
						'type' => 'text',
						'label' => $this->l('Speed'),
						'name' => 'CAMOSLIDER_SPEED',
						'suffix' => 'milliseconds',
	...
	public function getConfigFieldsValues()
	{
		...
		return array(
			'CAMOSLIDER_WIDTH' => Tools::getValue('CAMOSLIDER_WIDTH', Configuration::get('CAMOSLIDER_WIDTH', null, $id_shop_group, $id_shop)),
			'CAMOSLIDER_SPEED' => Tools::getValue('CAMOSLIDER_SPEED', Configuration::get('CAMOSLIDER_SPEED', null, $id_shop_group, $id_shop)),
			'CAMOSLIDER_PAUSE' => Tools::getValue('CAMOSLIDER_PAUSE', Configuration::get('CAMOSLIDER_PAUSE', null, $id_shop_group, $id_shop)),
			'CAMOSLIDER_LOOP' => Tools::getValue('CAMOSLIDER_LOOP', Configuration::get('CAMOSLIDER_LOOP', null, $id_shop_group, $id_shop)),
		);
	}
	...
	public function getAddFieldsValues()
	{
		...
		foreach ($languages as $lang)
		{
			...
			$fields['image_mobile'][$lang['id_lang']] = Tools::getValue('image_mobile'.(int)$lang['id_lang']);
